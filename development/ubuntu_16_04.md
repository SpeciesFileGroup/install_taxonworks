...
```
sudo apt-get install -y postgresql-9.4 postgresql-contrib-9.4 libgeos-dev libproj-dev postgresql-9.4-postgis-2.3 postgresql-9.4-postgis-2.3-scripts libpq-dev cmake imagemagick libmagickwand-dev git meld
```
...
rake db:seed
```
The username for the dummy account is `person1@example.com` and password is `Abcd123!`. Note, this account is a regular user and does not have admin privileges.
